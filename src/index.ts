...
async function removeAlreadyName(
  ...
  importComponents.filter((comp) => {
    ...
    if (importStr) {
      const cssFile = resolveNodeModules(root, modules ?? '', importStr!);
...
function transformComponentCss(root: string, lib: Lib, importVariables: readonly string[]) {
  ...
  for (let index = 0; index < importVariables.length; index++) {
    ...
    if (isPnp) {
      ...
      if (esModule) {
        importStr = resolveNodeModules(root, modules ?? '', importStr);
      }

      if (ensureStyleFile) {
        isAdd = ensureFileExists(root, modules ?? '', importStr, esModule);
      }
    }
...
function ensureFileExists(root: string, modules: string, importStr: string, esModule = false) {
...
function tryEnsureFile(root: string, modules: string, filePath: string, esModule = false) {
